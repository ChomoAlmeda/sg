...
/** Funciones **/
	...
	function get_lista( $id ) {
		...
		$this->db->order_by('au_lista_verificacion2.Requisito');
		$consulta = $this->db->get_where( 'au_lista_verificacion2', $condicion );
	...
	function get_lista_usuario_proceso( $id_auditoria, $id_proceso ) {
		$condicion = array(
			'au_lista_verificacion_usuario.IdProceso'		=> $id_proceso,
		...
		$this->db->join( 'au_lista_verificacion2', 'au_lista_verificacion2.IdListaVerificacion = au_lista_verificacion_usuario.IdListaVerificacion' );
		$consulta = $this->db->get_where( 'au_lista_verificacion_usuario', $condicion );
	...
	function get_respuestas_lista( $id_auditoria, $id_proceso ) {
		if( $this->session->userdata( 'id_area') != 9 ) {
			...
			$this->db->join( 'au_lista_verificacion2', 'au_lista_verificacion2.IdListaVerificacion = au_lista_verificacion_usuario.IdListaVerificacion' );
			$this->db->select( 'au_respuestas_lista.IdRespuestaLista, au_respuestas_lista.IdListaVerificacionUsuario, au_respuestas_lista.IdProceso, au_respuestas_lista.Tipo, au_respuestas_lista.Hallazgo, au_procesos.Proceso, au_lista_verificacion2.Requisito' );
			...
			$this->db->select( 'au_respuestas_lista_sibib.IdRespuestaLista, au_respuestas_lista_sibib.IdListaVerificacionUsuario, au_respuestas_lista_sibib.IdBiblioteca, au_respuestas_lista_sibib.Tipo, au_respuestas_lista_sibib.Hallazgo, ab_bibliotecas.Biblioteca AS Proceso, au_lista_verificacion.Requisito' );
	...
	//
	// get_conformidades(): Obtiene las no conformidades derivadas de los hallazgos
	//
	function get_conformidades() {
		// sigc
		if( $this->session->userdata( 'id_area') != 9 ) {
			$consulta = $this->db->get( 'pa_conformidades_hallazgos' );
		}
		// sibib
		else {
			$consulta = $this->db->get( 'pa_conformidades_hallazgos_sibib' );
		}
	...
	function get_lista_hallazgo( $id ) {
		...
		$this->db->join( 'au_lista_verificacion', 'au_lista_verificacion.IdListaVerificacion = au_lista_verificacion_usuario.IdListaVerificacion', 'LEFT' );
		$this->db->join( 'au_procesos_instructivos', 'au_procesos_instructivos.IdProceso = '.$tabla_respuestas.'.'.$tipo, 'LEFT' );
		$this->db->join( 'bc_documentos', 'bc_documentos.IdDocumento = au_procesos_instructivos.IdInstructivo', 'LEFT' );
		$this->db->join( 'ab_areas', 'ab_areas.IdArea = bc_documentos.IdArea', 'LEFT' );
	...
	//
	// get_biblioteca( $id ): Obtiene una biblioteca
	//	
	function get_biblioteca( $id ) {
		$condicion = array(
			'ab_bibliotecas.IdBiblioteca' => $id
		);
		$consulta = $this->db->get_where( 'ab_bibliotecas', $condicion, 1 );
		
		return $consulta;
	}
	
	//
	// get_area_proceso( $id ): Obtiene el área de un proceso
	//	
	function get_area_proceso( $id ) {
		$condicion = array(
			'au_procesos_instructivos.IdProceso' => $id,
		);
		
		$this->db->join( 'bc_documentos', 'bc_documentos.IdArea = ab_areas.IdArea');
	...
	function get_procesos_documentos( $id ) {
		$this->db->group_by('bc_documentos.IdDocumentos');
	...
	function get_jefes($id ){
		$condicion = array(
			'au_equipos_usuarios.IdUsuario'			=> $this->session->userdata( 'id_usuario' ),
			
			'au_equipos_usuarios.Tipo'				=> 'Jefe de Equipo Auditor',
		);
		$consulta=$this->db->get_where('au_equipos_usuarios', $condicion);
		
		return $consulta;
	}

	function get_hallazgos($id ){
			$condicion = array(
				'au_equipos_usuarios.IdEquipo'			=> $id,
			);
			$this->db->join('au_respuestas_lista', 'au_equipos_usuarios.IdUsuario=au_respuesta_lista.IdUsuario');
			$consulta=$this->db->get_where('au_equipos_usuarios', $condicion);
	...
	function inserta_actualiza_respuestas( $id ) {
		...
		if( $this->session->userdata( 'id_area') != 9 ) {				
			...
			if( $consulta->num_rows() > 0 ) {
				$i = 0;
				$inserta = array();
				$lista = $this->get_lista_usuario_proceso( $id, $this->input->post( 'proceso_hidden' ) );
